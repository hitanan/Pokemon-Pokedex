$(function(){
        ...
        onSelect: function (suggestion) {
            ...
            chrome.storage.local.get({'ids':[]}, function(pokemons) {
                ...
                chrome.storage.local.set(pokemons, function() {
                    var thehtml = '<strong>Pokemon Name:</strong> ' + suggestion.value + '<strong>, #:</strong> ' + suggestion.data;
    ...
    $(document).on('click','#pokelinks a', function(e) {
        ...
        //createTab( $(this).attr('href'), PATTERN);
        //return false;
    });

    // Load recent pokemons
    chrome.storage.local.get( {'ids':[]} , function(pokemons) {
        var recentPokemons = '<h2>Recent Pokemons:</h2><ul id=pokelinks">';
        for (var i = 0; i < pokemons.ids.length; i++) {
            recentPokemons += '<li><div class="pokemon-id">' + pokemons.ids[i] + '.</div><a href="' +PATTERN +pokemons.ids[i] +'#movesets" target="_blank"><strong>' + pokemons.ids[i] + '</strong></a></li>';
        }
        recentPokemons += "</ul>";
